...
public class MovieServiceImpl implements MovieService {
    ...
    @Autowired
    public MovieServiceImpl(MovieRepository movieRepository, ModelMapper modelMapper, GenreRepository genreRepository, ApplicationEventPublisher eventPublisher) {
    ...
    public void addMovie(CreateOrUpdateMovieDTO addMovieDTO) {
        ...
        MovieAddedEvent movieAddedEvent = new MovieAddedEvent(this, newMovie);
        eventPublisher.publishEvent(movieAddedEvent);
    }
    ...
    public List<MovieDTO> searchMovies(String query) {
        return movieRepository.findByNameContainingIgnoreCase(query)
                .stream()
                .map(movieEntity -> this.modelMapper.map(movieEntity,MovieDTO.class))
                .collect(Collectors.toList());
    }
